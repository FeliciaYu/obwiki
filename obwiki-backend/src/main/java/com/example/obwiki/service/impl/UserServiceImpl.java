...
import com.example.obwiki.resp.UserGetKeyResp;
...
private static final Logger LOG = Logger.getLogger(UserServiceImpl.class.getName());
    ...
    public UserLoginResp login(UserLoginReq req) {
        ...
        if (ObjectUtils.isEmpty(user)) {
            ...
            if (user.getPassword().equals(req.getPassword())) {
                ...
                // 密码不对
                LOG.info("密码不对, 输入密码：" + req.getPassword() + ", 数据库密码：" + user.getPassword());
                throw new BusinessException(BusinessExceptionCode.LOGIN_USER_ERROR);
            }
        }
    }
    ...
    public UserGetKeyResp getKeyByName(String loginName) {
        User one = this.baseMapper.selectOne(new QueryWrapper<User>().eq("login_name",loginName));
        if(one.getSalt()!=null){
            UserGetKeyResp resp = new UserGetKeyResp();
            resp.setSalt(one.getSalt());
            return resp;
        }else{
            throw new BusinessException(BusinessExceptionCode.USER_SALT_NOT_EXIST);
        }
    }
